...
void AndroidHostInterface::LoadSettings()
{
  ...
  CommonHostInterface::UpdateInputMap();
}
...
{
  JNIEnv* thread_env;
  __android_log_print(ANDROID_LOG_INFO, "AndroidHostInterface", "Attaching thread %s", boot_params->filename.c_str());
  ...
  __android_log_print(ANDROID_LOG_INFO, "AndroidHostInterface", "Creating ImgUi context %s", boot_params->filename.c_str());
  ...
  {
    __android_log_print(ANDROID_LOG_INFO, "AndroidHostInterface", "Booting %s", boot_params->filename.c_str());
    //auto params = std::make_shared<SystemBootParameters>();//boot_params.filename);
    boot_result = BootSystem(std::move(boot_params));
  }
...
void AndroidHostInterface::EmulationThreadLoop()
{
  for (;;)
  {
    ...
    if (System::IsRunning())
    {
      u16 TrackX = 0;
      u16 TrackY = 0;
      if(lTrackBallDirection[DIRECTION_UP] != 0){
        TrackY = -lTrackBallDirection[DIRECTION_UP];
      }
      if(lTrackBallDirection[DIRECTION_DOWN] != 0){
        TrackY = lTrackBallDirection[DIRECTION_DOWN];
      }
      if(lTrackBallDirection[DIRECTION_LEFT] != 0){
        TrackX = lTrackBallDirection[DIRECTION_LEFT];
      }
      if(lTrackBallDirection[DIRECTION_RIGHT] != 0){
        TrackX = -lTrackBallDirection[DIRECTION_RIGHT];
      }
      //__android_log_print(ANDROID_LOG_INFO, "TrackBall", "TrackBallKeys: %x, X: %x, Y: %x", gTrackBallKeys, TrackX, TrackY);
      KonamiTrackballSetXY(TrackX, TrackY);
...
bool AndroidHostInterface::AcquireHostDisplay()
{
  ...
  if (!display->CreateRenderDevice(wi, {}, g_settings.gpu_use_debug_device, true) ||
      !display->InitializeRenderDevice(GetShaderCacheBasePath(), g_settings.gpu_use_debug_device, true))
  {
...
void AndroidHostInterface::OnRunningGameChanged()
{
  ...
  CommonHostInterface::OnRunningGameChanged(title_string);
  ApplySettings(true);
...
extern "C" jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
  ...
  if ((string_class = env->FindClass("java/lang/String")) == nullptr ||
      ...
      (host_interface_class = env->FindClass("umido/androidemu/mame/AndroidHostInterface")) == nullptr ||
      (s_AndroidHostInterface_class = static_cast<jclass>(env->NewGlobalRef(host_interface_class))) == nullptr ||
      (patch_code_class = env->FindClass("umido/androidemu/mame/PatchCode")) == nullptr ||
  ...
  if ((s_AndroidHostInterface_constructor =
      ...
      (emulation_activity_class = env->FindClass("umido/androidemu/mame/EmulationActivity")) == nullptr ||
...
#define DEFINE_JNI_METHOD(return_type, name)                                                                           \
  extern "C" JNIEXPORT return_type JNICALL Java_umido_androidemu_mame_##name(JNIEnv* env)

#define DEFINE_JNI_ARGS_METHOD(return_type, name, ...)                                                                 \
  extern "C" JNIEXPORT return_type JNICALL Java_umido_androidemu_mame_##name(JNIEnv* env, __VA_ARGS__)
...
{
  return env->NewStringUTF(SmallString::FromFormat("Arcade1Up %s (%s)\nBuilt %s %s", g_scm_tag_str,
                       ...
                       jstring user_directory)
{
  ...
  if (!cpp_obj->Initialize())
  {
    ...
    __android_log_print(ANDROID_LOG_INFO, "AndroidHostInterface", "Initialized");
                       ...
                       jobject surface, jstring filename, jboolean resume_state, jstring state_filename)
{
  ...
  std::shared_ptr<SystemBootParameters> boot_params = std::make_shared<SystemBootParameters>();
...
DEFINE_JNI_ARGS_METHOD(void, AndroidHostInterface_shutdown, jobject obj)
{
    AndroidHelpers::GetNativeClass(env, obj)->Shutdown();
}
...
{
  /*jclass entry_class = env->FindClass("umido/androidemu/mame/GameListEntry");
...
DEFINE_JNI_ARGS_METHOD(jint, AndroidHostInterface_getMediaPlaylistIndex, jobject obj)
{
  ...
  return 0;//System::GetMediaPlaylistIndex();
}

DEFINE_JNI_ARGS_METHOD(jboolean, AndroidHostInterface_setMediaPlaylistIndex, jobject obj, jint index)
{
  if (!System::IsValid() || index < 0 || static_cast<u32>(index) >= 0)//System::GetMediaPlaylistCount())
    return false;
