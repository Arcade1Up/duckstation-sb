...
#define UART_DEV_NAME    "/dev/ttyS2"//ttyUSB0"    // Serial device
...
int lTrackBallDirection[4] = {0,0,0,0};//UP/DOWN/LEFT/RIGHT
...
void *TrackBallThread(void *ptr){
    ...
    if(fdSerial < 0)
    {
        ...
        __android_log_print(ANDROID_LOG_ERROR,  "TrackBall", "Unable to open %s!", UART_DEV_NAME);
        return NULL;
    }
    ...
    {
        __android_log_print(ANDROID_LOG_ERROR, "TrackBall", "UART fcntl F_SETFL failed!\n");
        close(fdSerial);
        fdSerial = -1;
        return NULL;
    }
    ...
    {
        __android_log_print(ANDROID_LOG_ERROR, "TrackBall", "UART set opt Error\n");
        close(fdSerial);
        fdSerial = -1;
        return NULL;
    }
    ...
    while(1)
    {
        ...
        if(lRecvLen > 0)
        {
           ...
           //Adjust start pos(first byte must is 0xFF)  
                ...
                switch(lData){
                    case 0x00FE://UP
                        ...
                        if(lTrackBallDirection[DIRECTION_UP] >= MINVALIDCNT){
                            lTrackBallDirection[DIRECTION_DOWN] = 0;
                        }
                        break;
                    case 0x0001://DOWN
                        lTrackBallDirection[DIRECTION_DOWN] += 1;
                        if(lTrackBallDirection[DIRECTION_DOWN] >= MINVALIDCNT){
                            lTrackBallDirection[DIRECTION_UP] = 0;
                        }
                        break;
                    case 0xFE00://LEFT
                        lTrackBallDirection[DIRECTION_LEFT] += 1;
                        if(lTrackBallDirection[DIRECTION_LEFT] >= MINVALIDCNT){
                            lTrackBallDirection[DIRECTION_RIGHT] = 0;
                        }
                        break;
                    case 0x0100://RIGHT
                        lTrackBallDirection[DIRECTION_RIGHT] += 1;
                        if(lTrackBallDirection[DIRECTION_RIGHT] >= MINVALIDCNT){
                            lTrackBallDirection[DIRECTION_LEFT] = 0;
                        }
                        break;
                    default:break;
                }
            ...
            //if(0x0 != gTrackBallKeys){
            //    printf("TRACKBALL Keys = 0x%x\n",gTrackBallKeys);
            //}
            //-----------------------------------------------------
        }
        else if(0 == lRecvLen)
        {
            ...
            if(1 == lLastRecvState){
                lLastRecvState = 0;
                //__android_log_print(ANDROID_LOG_ERROR, "TrackBall", "TIMEOUT: Cannot receive data\n");
            }
        }
        else
        {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "TrackBall", "Error: Receive data\n");
        }
    }
...
int Trackball_Start()
{
    ...
    while(1){
        ...
        if(0x0 != gTrackBallKeys){
            printf("--------%d\n",lFrameCnt);
            if(gTrackBallKeys & HARD_KEY_LEFT || gTrackBallKeys & HARD_KEY_UP){
                if(gTrackBallKeys & HARD_KEY_UP){
                    printf("  ↑ \n");
                }
                if(gTrackBallKeys & HARD_KEY_LEFT){
                    printf("← \n");
                }
            }
            if(gTrackBallKeys & HARD_KEY_DOWN || gTrackBallKeys & HARD_KEY_RIGHT){
                if(gTrackBallKeys & HARD_KEY_RIGHT){
                    printf("    → \n");
                }
                if(gTrackBallKeys & HARD_KEY_DOWN){
                    printf("  ↓ \n");
                }
            }
            //printf("Keys = 0x%x\n",gTrackBallKeys);
