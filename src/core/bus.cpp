...
ALWAYS_INLINE static TickCount DoBIOSAccess(u32 offset, u32& value)
{
  ...
  return m_bios_access_time[static_cast<u32>(size)];
    ...
    template<MemoryAccessType type, MemoryAccessSize size>
    static ALWAYS_INLINE TickCount DoScsiAccess(u32 offset, u32& value)
    {
      if constexpr (type == MemoryAccessType::Read)
      {
        KonamiScsiRead(1U << (u32)size, offset, value);
      }
      else
      {
        KonamiScsiWrite(1U << (u32)size, offset, value);
      }
      return m_exp1_access_time[static_cast<u32>(size)];
    }

    template<MemoryAccessType type, MemoryAccessSize size>
    static ALWAYS_INLINE TickCount DoP1Access(u32 offset, u32& value)
    {
      if constexpr (type == MemoryAccessType::Read)
      {
        KonamiP1Read(1U << (u32)size, offset, value);
      }
      else
      {
        KonamiP1Write(1U << (u32)size, offset, value);
      }
      return m_exp1_access_time[static_cast<u32>(size)];
    }

    template<MemoryAccessType type, MemoryAccessSize size>
    static ALWAYS_INLINE TickCount DoP2Access(u32 offset, u32& value)
    {
      if constexpr (type == MemoryAccessType::Read)
      {
        KonamiP2Read(1U << (u32)size, offset, value);
      }
      else
      {
        KonamiP2Write(1U << (u32)size, offset, value);
      }
      return m_exp1_access_time[static_cast<u32>(size)];
    }
    ...
    template<MemoryAccessType type, MemoryAccessSize size>
    static ALWAYS_INLINE TickCount DoEepromAccess(u32 offset, u32& value)
    {
      if constexpr (type == MemoryAccessType::Read)
      {
        KonamiEepromRead(1U << (u32)size, offset, value);
      }
      else
      {
        KonamiEepromWrite(1U << (u32)size, offset, value);
      }
      return m_exp1_access_time[static_cast<u32>(size)];
    }
    ...
    static ALWAYS_INLINE TickCount DoTrackballAccess(u32 offset, u32& value)
    {
      if constexpr (type == MemoryAccessType::Read)
      {
        KonamiTrackballRead(1U << (u32)size, offset, value);
      }
      else
      {
        KonamiTrackballWrite(1U << (u32)size, offset, value);
      }
      return m_exp1_access_time[static_cast<u32>(size)];
    }

    template<MemoryAccessType type, MemoryAccessSize size>
    static ALWAYS_INLINE TickCount DoScsiIrqAccess(u32 offset, u32& value)
    {
      if constexpr (type == MemoryAccessType::Read)
      {
        value = 0;
...
ALWAYS_INLINE static TickCount DoMemoryControlAccess(u32 offset, u32& value)
{
  ...
  {
    if (value == 0x173f47 && CPU::g_state.regs.pc == 0x80064e80)
    {
      KonamiScoreInit();
    }
...
static ALWAYS_INLINE TickCount DoMemoryAccess(VirtualMemoryAddress address, u32& value)
{
  ...
  else if (address < (EXP1_BASE + EXP1_SIZE))
  {
    ...
    {
      return DoEepromAccess<type, size>(address & EXP1_MASK, value);
    }
    if (address >= 0x1F6800C0 && address <= 0x1F6800C9)
    {
